...
export const useInitialLoader = () => {
  ...
  useEffect(() => {
    ...
    const minLoadingTime = 8000; // Минимум 8 секунд
    ...
    // Проверяем готовность каждые 300мс для оптимальной производительности
    const checkInterval = setInterval(() => {
      const elapsed = Date.now() - startTime;
      const resourcesReady = checkResourcesLoaded();

      // Условие завершения:
      // 1. Прошло минимум 8 секунд И ресурсы загружены
      // ИЛИ
    ...
    }, 300); // Оптимизированная частота проверки
